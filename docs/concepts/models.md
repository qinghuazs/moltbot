...
参见 [/concepts/model-failover](/concepts/model-failover) 了解认证配置文件轮换、冷却时间以及它们如何与回退交互。
...
相关：
- `agents.defaults.models` 是 Moltbot 可以使用的模型允许列表/目录（加上别名）。
...
这发生在生成正常回复**之前**，因此消息可能感觉像"没有响应"。修复方法是：
...
显示解析后的主要模型、回退、图像模型以及已配置提供商的认证概览。它还显示在认证存储中找到的配置文件的 OAuth 过期状态（默认在 24 小时内警告）。`--plain` 仅打印解析后的主要模型。
OAuth 状态始终显示（并包含在 `--json` 输出中）。如果已配置的提供商没有凭据，`models status` 打印 **Missing auth** 部分。
JSON 包含 `auth.oauth`（警告窗口 + 配置文件）和 `auth.providers`（每个提供商的有效认证）。
使用 `--check` 进行自动化（缺失/过期时退出 `1`，即将过期时退出 `2`）。

首选的 Anthropic 认证是 Claude Code CLI setup-token（在任何地方运行；如果需要，在网关主机上粘贴）：
...
`moltbot models scan` 检查 OpenRouter 的**免费模型目录**，并可以选择性地探测模型的工具和图像支持。
...
`models.providers` 中的自定义提供商被写入代理目录下的 `models.json`（默认 `~/.clawdbot/agents/<agentId>/models.json`）。除非 `models.mode` 设置为 `replace`，否则此文件默认合并。
